    ...
    return (
        <main className={`w-fill min-h-screen flex flex-row ${styles.bg}`}>
            ...
            >
                ...
                {!showModules && (
                    <p className="text-lg font-light md:text-3xl" style={{ transition: "opacity 1s ease", opacity: subHOpacity }}>
                        Brilliance Made Simple
                    </p>
                )}
                {showModules && (
                    <div className="w-full h-full overflow-y-scroll px-[20px]">
                        <div className="flex flex-row flex-wrap overflow-visible md:flex-nowrap gap-[25px]">
                            <Nav />
                            {currentPage === "dashboard" && (
                                <div 
                                    className={`w-full h-fit md:h-full text-black p-[20px] ${styles.dashboard}`} 
                                    style={{ borderRadius: "20px", transition: "opacity 1s ease", opacity: moduleOpacity }}
                                />
                            )}
                            {currentPage === "account" && (
                                <div 
                                    className={`w-full h-fit text-black p-[20px] bg-white ${styles.page}`} 
                                    style={{ borderRadius: "20px", transition: "opacity 1s ease", opacity: moduleOpacity }}
                                >
